...
class VAEG(VAEGConfig):
    def __init__(self, hparams, placeholders, num_nodes, num_features, edges, negedges, istest=False):
        ...
        def masked_gen(posscore, negscore):
            ...
            temp_posscore = tf.reduce_sum(posscorestack)
            #temp_negscore = tf.reduce_sum(negscorestack)
            ll = 0.0
            '''
            for (u, v) in self.edges[self.count]:
                ...
                ll += tf.log(tf.multiply(tf.reshape(posscore[u], [1, self.n]), indicator[u])[0][v] / (temp_posscore + temp_negscore)+1e-09)
                ll += tf.log(tf.multiply(tf.reshape(posscore[v], [1, self.n]), indicator[v])[0][u] / (temp_posscore + temp_negscore)+1e-09)
    ...
    def initialize(self):
        ...
        print("Time take to initialise parameters", (end - start))
    ...
    def sample_graph(self, hparams, placeholders, s_num, node, num=10, outdir=None, eps_passed=None):
	    ...
	    feed_dict.update({self.features: features[i]})
            ...
            print "Time to lead at sampling approach1", end - start
            ...
            for (u,v) in candidate_edges:
                #probtotal *= prob[u][v]
                #print "Sample_file", hparams.sample_file, s_num
            ...
            print "Time to sample a graph", end - start, "with edges", len(candidate_edges)
	...
	feed_dict.update({self.features:features[0] })
        ...
        print "Time to load model approach 2", end - start
        ...
        print "Time to sample a graph approach 2", end - start, "with edges", len(candidate_edges)
