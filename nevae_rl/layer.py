...
def input_layer(c_mat, adj,weight, feature, k,n,d,activation = None, batch_norm = False, istrain = False, scope = None):
    ...
    w_in = tf.get_variable(name="w_in", shape=[k,d,d], initializer=tf.constant_initializer(0.5), dtype=tf.float32)
...
def fc_layer(input_, output_size, activation = None, batch_norm = False, istrain = False, scope = None):
    ...
    with tf.variable_scope(scope or "fc"):
        w = tf.get_variable(name="w", shape = [get_shape(input_)[1], output_size], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32)
	...
	#w = tf.get_variable(name="w", shape = [get_shape(input_)[1], output_size], initializer=tf.constant_initializer(0.0001))
	    ...
	    b = tf.get_variable(name="b", shape = [output_size], initializer=tf.constant_initializer(0.01), dtype=tf.float32)
