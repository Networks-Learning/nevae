...
class VAEGCell(object):
	...
	with tf.variable_scope(scope or type(self).__name__):
            appended_features = tf.concat((self.features, self.coord), axis = 1)
	    # after concatenating for each coordinate one dimension will be changed
	    c_x = input_layer(c_x, self.adj, self.weight, appended_features, k, n, d+3, activation=None, batch_norm=False, istrain=False, scope=None)
        ...
        return (enc_mu, enc_sigma, z, coor_mu, coor_sigma_list)
