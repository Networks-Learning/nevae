...
def load_data(filename, num):
    ...
    featurelist = []
    edgelist = []
    for findex in range(0,5000):
        #for findex in range(1,21):
        #for findex in range(20):
        #for findex in range(20):
        fname = path+str(findex)+".edgelist"
        #fname = path+str(findex)+".txt"
        #fname = path+str(findex+1)+".txt"
        ...
        for u in G.nodes():
            ...
            degreemat[int(u)][0] = G.degree(u)/(n-1)
        ...
        #print("Check", bfs_edgelist+diff_edges)
        edgelist.append(bfs_edgelist+diff_edges)
        #print(bfs_edges, diff_edges)
        #except:
        #continue
    #print("Debug edge", edgelist)
